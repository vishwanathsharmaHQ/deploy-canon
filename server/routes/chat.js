...
const { getNeo4j, toNum, getSession } = require('../db/driver');
const { getNextId, formatThread, formatNode, NODE_TYPES } = require('../db/queries');
...
const { extractContentText } = require('../services/contentParser');
  ...
  try {
    ...
    try {
      ...
      if (usedResponsesApi) {
        ...
        send({ type: 'error', error: responsesErr.message });
        closeStream();
        return;
      }
...
- If the question is about an existing node (nodeContext provided), and the reply suggests updating that node, set proposedUpdate with the updated fields.`;
    ...
    try {
      ...
      extracted = { topicShift: false, nodes: [] };
    }
    ...
    const proposedNodes = (extracted.nodes || [])
      .filter((n) => NODE_TYPES.includes(n.type) && n.title && n.content)
      .map((n) => ({
        type: n.type,
        title: n.title,
        content: n.content,
