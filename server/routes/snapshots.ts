...
import { getNextId, formatNode } from '../db/queries.js';
import { requireAuth } from '../middleware/auth.js';
import { withSession } from '../middleware/session.js';
import type { SnapshotNode, TimelineEvent } from '../types/domain.js';
...
router.get('/:threadId/snapshots/diff', withSession(async (req, res) => {
  ...
  const result = await req.neo4jSession!.run(
    ...
    { threadId: getNeo4j().int(threadId), v1: getNeo4j().int(parseInt(v1 as string)), v2: getNeo4j().int(parseInt(v2 as string)) }
  );
  ...
  const ids1 = new Set(snap1.map((n: SnapshotNode) => n.id));
  const ids2 = new Set(snap2.map((n: SnapshotNode) => n.id));
  const map1: Record<number, SnapshotNode> = Object.fromEntries(snap1.map((n: SnapshotNode) => [n.id, n]));

  const added = snap2.filter((n: SnapshotNode) => !ids1.has(n.id));
  const removed = snap1.filter((n: SnapshotNode) => !ids2.has(n.id));
  const modified = snap2.filter((n: SnapshotNode) => ids1.has(n.id) && (map1[n.id!].title !== n.title || map1[n.id!].content !== n.content));
