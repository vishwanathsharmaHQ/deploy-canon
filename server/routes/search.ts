...
router.get('/semantic', withSession(async (req, res) => {
  ...
  const k = Math.min(parseInt(limit as string), 50);
  let threads: ReturnType<typeof formatThread>[] = [];
  let nodes: (ReturnType<typeof formatNode> & { relevance: number; threadId: number | null; threadTitle: string })[] = [];

  const queryEmbedding = await generateEmbedding(q as string).catch(() => null);
  if (queryEmbedding) {
...
router.post('/contradictions', requireAuth, aiTimeout, withSession(async (req, res) => {
  ...
  for (const record of nodesResult.records) {
    ...
    const simScoreMap: Record<string, number> = {};
